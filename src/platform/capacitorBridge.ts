...
interface CapacitorGlobal {
  ...
  Plugins?: {
    App?: {
      addListener: (event: 'backButton', callback: () => void) => CapacitorBackButtonHandle | Promise<CapacitorBackButtonHandle>;
...
// window.close() (used for the web/Yandex portal build) has no effect on a
// native Android app at all — it's a browser-tab API, and there's no browser
// tab here. App.exitApp() is Capacitor's documented way to actually terminate
// the app's Activity; per Capacitor's own docs it "should only be used in
// conjunction with the backButton handler for Android to exit the app when
// navigation is complete" — exactly this project's exitConfirm flow, reached
// via useCapacitorBackButton above. iOS doesn't support it, which doesn't
// matter here: this project only ships natively on Android (RuStore/Google
// Play), never iOS.
export function exitNativeApp() {
  capacitor()?.Plugins?.App?.exitApp().catch(() => undefined);
}
