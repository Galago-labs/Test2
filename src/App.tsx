...
import { hasShownFirstSessionAd, markFirstSessionAdShown } from './game/adSchedule';
...
import { exitNativeApp, isCapacitorNative, useCapacitorBackButton } from './platform/capacitorBridge';
...
export default function App() {
  ...
  // Two deliberate ad moments, not "every round" (that was reconsidered as
  // too much for a ~60-second game — see maybeShowFirstSessionAd below and
  // the dreamReveal-close trigger further down, near onConfirmExit).
  const maybeShowFirstSessionAd = useCallback(() => {
    if (hasShownFirstSessionAd()) return;
    markFirstSessionAdShown();
    // Yandex requirement: interstitials belong at natural pauses, and sound
    // must be muted while one is on screen and restored after.
    platform.showInterstitial(() => audio.muteImmediately(), () => audio.resumeFromInterrupt());
  }, [platform.showInterstitial, audio]);
  const recordFinished = useCallback((result: GameSnapshot) => {
    ...
    // Covers "played a full round and it naturally ended" — a round quit
    // mid-way instead reaches this same one-time ad through onReset below.
    maybeShowFirstSessionAd();
  }, [recordRound, platform.setGameplay, maybeShowFirstSessionAd]);
  ...
  const closeModal = useCallback((startsNap = false) => {
    ...
    // The dream reveal is a reward for reaching 5 wins in a scenario — at
    // most 3 times ever, once per scenario. Never gate it behind an ad or
    // interrupt it with one; only once the player has actually finished
    // looking at it and is leaving (whether via its own button, the dialog's
    // X, or Escape/back — closeModal is the one path all of those share).
    if (modal === 'dreamReveal') platform.showInterstitial(() => audio.muteImmediately(), () => audio.resumeFromInterrupt());
    dispatchDialog({ type: 'close', startsNap });
  }, [modal, dialogExiting, platform.showInterstitial, audio]);
      ...
      <main className="game-stage" style={layout ? { width: layout.width, height: layout.height } : undefined} aria-label={t('game.label')}>
        ...
        <div className="game-content" inert={controlsBlocked}>
          <div ref={surfaceRef} className="game-surface" tabIndex={0} role="application" aria-label={t('game.instructions')} {...input}>
            ...
            {uiReady && <>
              ...
              <GameOverlays game={game} t={t} onStart={requestNap} onResume={resumeNap} onReset={() => { queuedStart.current = false; platform.setGameplay(false); if (game.status === 'paused') maybeShowFirstSessionAd(); reset(); focusGame(); }} onJournal={() => openModal('journal')} onSettings={() => openModal('settings')} newBest={finished && game.score > roundBest.current} unlockedCount={roundUnlocks.length} onRoom={() => openModal('room')} decorationCount={decorationCount} completedScenario={completedScenario} onDreamReveal={() => openModal('dreamReveal')} progress={progress} />
        ...
        <AnimatePresence mode="wait" onExitComplete={() => dispatchDialog({ type: 'closed', revision: dialogs.revision })}>
          {modal && <LittleDialog
            ...
            onConfirmExit={() => { if (isNativeApp) { exitNativeApp(); return; } try { window.close(); } catch { /* Most browser contexts ignore this; the portal/TV shell owns the actual exit there. */ } }}
          />}
        </AnimatePresence>
        </div>
      </main>
